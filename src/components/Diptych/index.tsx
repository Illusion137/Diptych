...
export default function Diptych() {
	...
	const [current_file_path, set_current_file_path] = useState<string | null>(
	...
	useEffect(() => {
		...
		const on_change = (markdown: string) => {
		...
		manager.create(editorRef.current, on_change).then(async () => {
			const path = localStorage.getItem(STORAGE_KEY);
			if (path) {
